...
#ifndef CASADI_SX_DECL_HPP
#define CASADI_SX_DECL_HPP
...
namespace casadi {
	...
	template<> bool Matrix<SXElem>::__nonzero__() const;
	...
	template<> std::vector<std::vector<SX> >
		...
		template<> std::vector<std::vector<SX> >
			...
			template<> void SX::print_split(casadi_int nnz, const SXElem* nonzeros, std::vector<std::string>& nz,
